                                ...
                                <div class=" form">
                                    <form class="cmxform form-horizontal tasi-form" method="POST" action="">
                                      <div class="form-group ">
                                          <label for="cemail" class="control-label col-lg-2">QQ</label>
                                          <div class="col-lg-10">
                                              <input class="form-control " name="qq" value="<?php echo $row["qq"]; ?>">
                                          </div>
                                      </div>
                                      <div class="form-group ">
                                      ...
                                      <div class="form-group ">
                                          <label for="cemail" class="control-label col-lg-2">多说ID<a href="http://i.chainwon.com/#!25" target="_blank"> <span class="badge bg-important">？</span></a></label>
                                          <div class="col-lg-10">
                                              <input class="form-control " name="dsuserid" value="<?php echo $row["dsuserid"]; ?>">
                                          </div>
                                      </div>
                                      <?php
                                      if (isset($_REQUEST['save'])){
                                        if(is_numeric($_POST['dsuserid'])){
                                      $nowqq = $_POST["qq"];
                                      $nowemail = $_POST["email"];
                                      $nowdsuserid = $_POST["dsuserid"];
                                      $uid = $row["uid"];
                                      $conwrite=mysqli_connect("localhost","root","Lzj021272333", "user");
                                      mysqli_query($conwrite,"UPDATE user SET qq = '".$nowqq."', email = '".$nowemail."', dsuserid = '".$nowdsuserid."'  WHERE uid = $uid");
                                      echo "<script> location.replace(location.href);</script>";
                                    }else{
                                      echo '<div class="form-group"><center>多说ID请输入数字</center></div>';
                                    }
                                      }
                                      ?>
                                            <div class="form-group">
